...
\startslide [title={Quick revision}]
	...
	\startitemize[n]
		\item  Properties of Brownian motion \m{B(t)}
			\startitemize[1, columns, two]
				\item  starts at 0
				\item  continuous paths
				\item  Markov (independence of increments)
				\item  \m{B(t) - B(s) ∼ N(0, t - s)}
				\item  infinite linear variation
				\item  finite quadratic variation
				\item  martingale
			\stopitemize
...
\startslide [title={Itô integral: \m{f ∈ L^2}}]
	...
	\startdefinition[title={Itô integral}]
		\m{f ∈ C[a, b]} is called a continuous function.
	\stopdefinition
