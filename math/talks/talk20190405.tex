...
\startslide [title={Quick revision and notations}]
	...
	\startitemize [4]
		...
		\item  Properties of \m{B_∙}
			\startitemize[9, columns, joinedup]
				\item  starts at 0
		...
		\item  A stochastic process \m{X_∙} is called adapted to \m{ℱ_∙} if \m{X_t} is measurable w.r.t. \m{ℱ_t} \m{∀t}.
...
\startslide [title={Wiener integral (\m{f ∈ L^2[0, T]})}]
	...
	\startitemize [4]
		...
		\item  The associated process \m{I_∙ = ∫_0^∙ f(t) \d B_t} has the following properties.
			\startitemize [9, joinedup]
				\item  continuity
				\item  martingale
			\stopitemize
	...
	\startitemize [4]
		...
		\item  Choosing different endpoints for \m{t_j^*} gives us different results.
			\starttabulate [|c|m|m|c|c|]
				...
				\FL
				\NC  \good{left}   \NC  \good{\frac12 \brnd[B_t^2 - t]}  \NC  \good{0}  \NC  \good{\smile}  \NC  \good{Itô}  \NR
				\NC  mid    \NC  \frac12 \brnd[B_t^2]      \NC  \frac12 t  \NC  \frown  \NC  Stratonovich  \NR
				\NC  \okay{right}  \NC  \okay{\frac12 \brnd[B_t^2 + t]}  \NC  \okay{t}  \NC  \okay{\frown}  \NC    \NR
				\BL
			\stoptabulate
...
\startslide [title={Itô integral for \m{X_∙ ∈ L^2_{\text{ad}} ([0, T] × Ω)}}]
	...
	\startitemize [4]
		...
		\item  The associated process \m{I_∙ = ∫_0^∙ X_t \d B_t} has the following properties.
			\startitemize [9, joinedup]
				\item  continuity
				\item  martingale
			\stopitemize
...
\startslide [title={Itô integral for \m{X_∙} such that \m{∫_0^T X_t^2 \d t < ∞} a.s.}]
...
\startslide [title={The new integral \cite[short][AyedKuo2008, AyedKuo2010]}: Idea]
	...
	\startitemize[1]
		...
		\item  A process \m{Y^⋅} and filtration \m{ℱ_∙} are called \important{instantly independent} if \m{Y^t} and \m{ℱ_t} are independent \m{∀ t}.
			...
			Example: The process \m{(B_T - B_∙)} is instantly independent of the filtration generated by \m{B_∙}.
...
\startslide [title={Generalized Itô formula \cite[short][HwangKuoSaitôZhai2016]}]
	...
	\starttabulate [|c|m|m|]
		...
		\NC  \good{Itô}
		\NC  X_t = X_0 + ∫_0^t m(s, X_s) \d s + ∫_0^t σ(s, X_s) \d B_s
		\NC  \d X_t = m(t, X_t) \d t + σ(t, X_t) \d B_t

		\NR

		\NC  \okay{instantly independent}
		\NC  Y^t = Y^0 + ∫_t^T \tilde{m}(s, Y^s) \d s + ∫_t^T \tilde{σ}(s, Y^s) \d B_s
		\NC  \d Y^t = \tilde{m}(t, Y^t) \d t + \tilde{σ}(t, Y^t) \d B_t
	...
	\starttheorem[title={\cite[short][HwangKuoSaitôZhai2016]}]
		Let \good{\m{\d X_t = m \d t + σ \d B_t}} be an \m{d}-dimensional \good{Itô} process, \okay{\m{\d Y^t = \tilde{m} \d t + \tilde{σ} \d B_t}} be a \m{\tilde{d}}-dimensional \okay{instantly independent} process.
