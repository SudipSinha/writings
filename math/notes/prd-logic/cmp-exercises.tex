...
\startexercises [title={\getvariable{document}{author}  \hfill  \getvariable{document}{course}: PS3  \hfill  2019-10-15}]
	...
	\startexercise [title={Enderton, page 87}]
		Prove that \m{\sementl[𝔄][s]{α ∨ β}} iff \m{\sementl[𝔄][s]{α}} or \m{\sementl[𝔄][s]{β}}.
	\stopexercise
	\startsolution
		\startformula  \startalign[n=3]
			\NC     \NC  \sementl[𝔄][s]{(α ∨ β)}  \NR
			\NC  ⟺  \NC  \sementl[𝔄][s]{(¬α →β)}  \NC  \qquad  [\text{Expansion of \m{∨}}]  \NR
			\NC  ⟺  \NC  \text{either }  \nsementl[𝔄][s]{¬α}  \text{ or }  \sementl[𝔄][s]{β} \NC  \qquad  [\text{Definition of \m{s} for \m{→}}]  \NR
			\NC  ⟺  \NC  \text{either }  \sementl[𝔄][s]{α} \text{ or }  \sementl[𝔄][s]{β} \NC  \qquad  [\text{Definition of \m{s} for \m{¬}}]  \NR
		\stopalign  \stopformula
	\stopsolution
\stopexercises
...
\startexercises [title={\getvariable{document}{author}  \hfill  \getvariable{document}{course}: PS4  \hfill  2019-10-29}]
			...
			\item  \m{Γ ∪ \bcrl[α] ⊨ ϕ} iff \m{Γ ⊨ (α → ϕ)}.
				\startsolution
					\startitemize
						...
						\sym{\bold{(⟹)}}  \qquad
							\m{Γ ∪ \bcrl[α] ⊨ ϕ} means that any structure \m{𝔄} and satisfaction function \m{s: V → \abs[𝔄]} such that \m{𝔄} satisfies every member of \m{Γ} and \m{α} with \m{s}, the structure \m{𝔄} also satisfies \m{ϕ} with \m{s}. Now, let \m{𝔄} and \m{s} be such a combination. Now, either \m{\nsementl[𝔄][s]{α}} or \m{\sementl[𝔄][s]{α}}. If \m{\nsementl[𝔄][s]{α}}, then \m{\sementl[𝔄][s]{α → ϕ}} by definition. If \m{\sementl[𝔄][s]{α}}, then \m{Γ ∪ \bcrl[α] ⊨ ϕ} and \m{\sementl[𝔄][s]{α}} implies \m{\sementl[𝔄][s]{(α → ϕ)}}. Therefore, \m{Γ ⊨ (α → ϕ)}.

						\sym{\bold{(⟸)}}  \qquad
							Since we have \m{Γ ⊨ (α → ϕ)}, any structure \m{𝔄} and satisfaction function \m{s: V → \abs[𝔄]} such that \m{𝔄} satisfies every member of \m{Γ} and \m{α} with \m{s}, since we have \m{\sementl[𝔄][s]{α}} and \m{Γ ⊨ (α → ϕ)}, it must be that \m{\sementl[𝔄][s]{ϕ}}.
					\stopitemize
				\stopsolution

			\item  \m{ϕ ⟚ ψ} iff \m{⊨ (ϕ ↔ ψ)}.
				\startsolution
					% For any structure \m{𝔄} and satisfaction function \m{s: V → \abs[𝔄]}, either \m{\nsementl[𝔄][s]{ϕ}} or \m{\sementl[𝔄][s]{ϕ}}.

					% If \m{\nsementl[𝔄][s]{ϕ}}, we have \m{\nsementl[𝔄][s]{ψ}}, so \m{\sementl[𝔄][s]{ϕ ↔ ψ}}.

					% If \m{\sementl[𝔄][s]{ϕ}}, we have \m{\sementl[𝔄][s]{ψ}}, so \m{\sementl[𝔄][s]{ϕ ↔ ψ}}.
					\startformula  \startalign[n=3]
						\NC     \NC  ϕ ⟚ ψ  \NR
						\NC  ⟺  \NC  ⊨ (ϕ → ψ) \text{ and }  ⊨ (ψ → ϕ)  \NC  [Using (a)]  \NR
						\NC  ⟺  \NC  \text{for every } (𝔄, s),  \sementl[𝔄][s]{(ϕ → ψ)}  \text{ and }  \sementl[𝔄][s]{(ψ → ϕ)}  \NR
						\NC  ⟺  \NC  \text{for every } (𝔄, s),  \sementl[𝔄][s]{(ϕ → ψ) ∧ (ψ → ϕ)}  \NC  [\text{Definition of } ∧]  \NR
						\NC  ⟺  \NC  \text{for every } (𝔄, s),  \sementl[𝔄][s]{(ϕ ↔ ψ)}  \NC  [\text{Definition of } ↔]  \NR
						\NC  ⟺  \NC  ⊨ (ϕ ↔ ψ)  \NR
					\stopalign  \stopformula
				\stopsolution
		\stopitemize
	...
	\startsolution
		...
		\placeformula  \startformula  \startalign
			\NC  \sementl[𝔄][s]{∀x\,α}        ⟺  \NC  \text{for every } a ∈ \abs[𝔄], \sementl[𝔄][s(x∣a)]{α} , \qquad \text{ and }  \NR[eq:condition1]
			\NC  \sementl[𝔄][s]{∀x\,(α → β)}  ⟺  \NC  \text{for every } a ∈ \abs[𝔄], \sementl[𝔄][s(x∣a)]{α → β}  \NR
			\NC  ⟺  \NC  \text{for every } a ∈ \abs[𝔄], \nsementl[𝔄][s(x∣a)]{α} \text{ or } \sementl[𝔄][s(x∣a)]{β} .  \NR[eq:condition2]
		\stopalign  \stopformula

		Fix an arbitrary \m{a ∈ \abs[𝔄]}. If \m{\nsementl[𝔄][s(x∣a)]{α}} in (\in[eq:condition2]), then combining it with (\in[eq:condition1]) gives a contradiction. Therefore, we must have \m{ \sementl[𝔄][s(x∣a)]{β}}. This means for every \m{a ∈ \abs[𝔄]}, we have \m{ \sementl[𝔄][s(x∣a)]{β}}, which says \m{\sementl[𝔄][s]{∀x\,β}}.
	\stopsolution
	...
	\startsolution
		\startformula  \startalign
			...
			\NC  ⟺  \NC  \text{for every } 𝔄 \text{ and } s: V → \abs[𝔄], \text{ we have } \sementl[𝔄][s]{θ}  \NR
			\NC  ⟺  \NC  \text{for every } 𝔄 \text{ and } s: V → \abs[𝔄] \text{ and } a ∈ \abs[𝔄], \text{ we have } \sementl[𝔄][s(x∣a)]{θ}  \NR
			\NC  ⟺  \NC  \text{for every } 𝔄 \text{ and } s: V → \abs[𝔄], \text{ we have } \sementl[𝔄][s]{∀x\,θ}  \NR
...
\startexercises [title={\getvariable{document}{author}  \hfill  \getvariable{document}{course}: PS5  \hfill  2019-11-05}]
	...
	\startexercise [title={Enderton, 2.2.18}]
		...
		\startitemize [a]
			...
			\item  Show that if \m{\sementl[𝔄][s]{ψ}} and \m{ψ} is existential, then \m{\sementl[𝔅][s]{ψ}}. And if \m{\sementl[𝔅][s]{ϕ}} and \m{ϕ} is universal, then \m{\sementl[𝔄][s]{ϕ}}.
				\startsolution
					...
					\startitemize [i]
						...
						\item  \bold{If \m{\sementl[𝔅][s]{ϕ}} and \m{ϕ} is universal, then \m{\sementl[𝔄][s]{ϕ}}.}

							Since \m{ϕ} is universal, \m{ϕ = ∀ x_1 ⋯ ∀ x_n \, θ} for some quantifier-free wff \m{θ}. Therefore, \m{\sementl[𝔅][s]{∀ x_1 ⋯ ∀ x_n \, θ}} means that for every \m{b_1, …, b_n ∈ \abs[𝔅]}, we have \m{\sementl[𝔅][s {\brnd[{\tuple[x_1, …, x_n]} ∣ {\tuple[b_1, …, b_n]}]}] {θ}}. Therefore, in particular, for every \m{a_1, …, a_n ∈ \abs[𝔄] ⊆ \abs[𝔅]}, we have \m{\sementl[𝔅][s {\brnd[{\tuple[x_1, …, x_n]} ∣ {\tuple[a_1, …, a_n]}]}] {θ}}. By the Homomorphism Theorem, for every \m{a_1, …, a_n ∈ \abs[𝔄]}, we have \m{\sementl[𝔄][s {\brnd[{\tuple[x_1, …, x_n]} ∣ {\tuple[a_1, …, a_n]}]}] {θ}}, that is, \m{\sementl[𝔄][s]{∀ x_1 ⋯ ∀ x_n \, θ}}.

						\item  \bold{If \m{\sementl[𝔄][s]{ψ}} and \m{ψ} is existential, then \m{\sementl[𝔅][s]{ψ}}.}

							Since \m{ψ} is existential, \m{ψ = ∃ x_1 ⋯ ∃ x_n θ = ¬∀x_1¬ ¬∀x_2¬ ⋯ ¬∀x_2¬ θ}, which is logically equivalent to saying \m{ψ = ¬∀x_1 ∀x_2 ⋯ ∀x_n ¬θ}. Therefore, \m{\sementl[𝔄][s]{ψ}} is the same as \m{\sementl[𝔄][s]{¬ ∀x_1 ∀x_2 ⋯ ∀x_n ¬θ}}, which is in turn the same as \m{\nsementl[𝔄][s]{∀x_1 ∀x_2 ⋯ ∀x_n ¬θ}}. Now, using the above result, we get that this implies \m{\nsementl[𝔅][s]{∀x_1 ∀x_2 ⋯ ∀x_n ¬θ}}. Reversing the above steps, we get \m{\sementl[𝔅][s]{ψ}}.
...
\startexercises [title={\getvariable{document}{author}  \hfill  \getvariable{document}{course}: PS6  \hfill  2019-11-19}]
	...
	\startexercise [title={Enderton, 2.4.3}]
		\startitemize [a]
			...
			\item[ex2.4.3a]   Let \m{𝔄} be a structure and let \m{s: V → \abs[𝔄]}. Define a truth assignment \m{v} on the set of prime formulas by
				\startformula
					v(α) = T  \qquad  \text{iff}  \qquad  \sementl[𝔄][s]{α} . 
				\stopformula
				Show that for any formula (prime or not),
				\startformula
					\bar{v}(α) = T  \qquad  \text{iff}  \qquad  \sementl[𝔄][s]{α} . 
				\stopformula
				\startsolution
					...
					\startitemize [i]
						...
						\item  \bold{Base case: \m{α} is prime.}  \qquad
							In this case, \m{\bar{v}(α) = v(α) = T} iff \m{\sementl[𝔄][s]{α}} by the given condition.
						...
						\item  \bold{Induction step: \m{α} is \m{¬β} for some \m{β}.}  \qquad
							\startformula  \startalign[n=3]
								...
								\NC  ⟺  \NC  \nsementl[𝔄][s]{β}  \qquad  \NC  [\text{Induction hypothesis}]  \NR
								\NC  ⟺  \NC  \sementl[𝔄][s]{¬β}  \NC  [\text{Definition of satisfaction}]  \NR
								\NC  ⟺  \NC  \sementl[𝔄][s]{α}  \NR
							\stopalign  \stopformula
						...
						\item  \bold{Induction step: \m{α} is \m{β → γ} for some \m{β} and \m{γ}.}  \qquad
							\startformula  \startalign[n=3]
								...
								\NC  ⟺  \NC  \nsementl[𝔄][s]{β}  \text{ or }  \sementl[𝔄][s]{γ}  \qquad  \NC  [\text{Induction hypothesis}]  \NR
								\NC  ⟺  \NC  \sementl[𝔄][s]{β → γ}  \NC  [\text{Definition of satisfaction}]  \NR
								\NC  ⟺  \NC  \sementl[𝔄][s]{α}  \NR
							\stopalign  \stopformula
					\stopitemize
				\stopsolution
...
\startexercises [title={\getvariable{document}{author}  \hfill  \getvariable{document}{course}: PS7  \hfill  2019-11-19}]
	...
	\startsolution
		...
		\startlemma
			\m{Γ ⊨ ϕ} iff \m{Γ ∪ ¬ϕ} is unsatisfiable
		\stoplemma
		\startproof
			\emph{Notation}: In what follows, we use \m{∃ \tuple[𝔄, s] \ P} to denote \quotation{there exists a structure \m{𝔄} and a satisfaction function \m{s:V → \abs[𝔄]} such that \m{P}}. We shall also write \m{\sementl[𝔄][s]{Γ}} to mean that \m{\sementl[𝔄][s]{γ}} for every \m{γ ∈ Γ}.
			
			\startformula  \startalign[n=3]
				\NC  Γ ⊭ ϕ  ⟺  \NC  ∃ \tuple[𝔄, s] (\sementl[𝔄][s]{Γ}  \text{ and }  \nsementl[𝔄][s]{ϕ})  \NC  [\text{Definition of satisfiability}]  \NR
				\NC  ⟺  \NC  ∃ \tuple[𝔄, s] (\sementl[𝔄][s]{Γ}  \text{ and }  \sementl[𝔄][s]{¬ϕ})  \NC  [\text{Definition of satisfaction function}]  \NR
				\NC  ⟺  \NC  ∃ \tuple[𝔄, s] (\sementl[𝔄][s]{Γ ∪ \bcrl[ϕ]}  \NR
				\NC  ⟺  \NC  Γ ∪ \bcrl[ϕ] \text{ is satisfiable} .  \NC  [\text{Definition of satisfiability}]  \NR
			\stopalign  \stopformula
			Moreover, \m{Δ} is satisfiable iff \m{Δ ∪ \bcrl[¬⊥]} is satisfiable iff \m{Δ ⊭ ⊥}.		  
		\stopproof
