...
\startslide[title={Martingales and Markov processes}]
	...
	\startdefinition
		...
		Let \m{X_{\argdotsub}} be an \m{L^1}-bounded \m{ℱ_{\argdotsub}}-adapted stochastic process and let \m{0 ≤ s ≤ t ≤ T}. Then \m{X_{\argdotsub}} is called a \important{martingale} if \m{𝔼(X_t ∣ ℱ_s) = X_s}.
...
\startslide [title={Definition of large deviation principle}]
	...
	\starttheorem [title={\cite[short][Cramér1938]}]
		Let \m{(X_n)} be a sequence of i.i.d. real random variables with finite moment generating function \m{M}. Then \m{(X_n)} follows the large deviation principle with rate function \m{I(x) = \sup_θ \brnd[θ x - \log M(θ)]}.
	\stoptheorem
\stopslide
