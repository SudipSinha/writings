	...
	\startitemize [m]
		...
		\item  Assume that we have a rectangular box of dimensions \m{l × b × h}.
			\startitemize [a, joinedup]
				...
				\item  By what factor must each factor be scaled so that the length of the diagonal is doubled?
			\stopitemize

		\item  Use the following formula for the sum of an infinite geometric series:
			\startformula
				a + a r + a r^2 + ⋯ = \frac{a}{1-r}, \qquad if \abs[r] < 1
			\stopformula
			\startitemize [a, joinedup]
...
\startslide [title={Proof of \m{2^n > n} using mathematical induction}]
	...
	\startproof
		...
		Since \m{k > 1}, \m{k + k > k + 1}. Therefore \m{2^{k + 1} > k + 1}.
...
\startslide [title={Exercises}]
	...
	\startitemize [m]
		...
		\item  Simplify each of the following sums to express it as a simple fraction:
			\startitemize [a, joinedup]
				\item  \m{\frac{1}{1 ⋅ 2}}
...
\startslide [title={Basics}]
	...
	\startdefinition [title={divisibility}]
		Let \m{a, d ∈ ℤ}. We say that \m{d} divides \m{a} if there exists \m{q ∈ ℤ} such that \m{a = q d}.

		We write \m{d ∣ a} and read it as \emph{\m{d} divides \m{a}}.
	\stopdefinition
	
	Exercises
	\startitemize [m]
		
		\item  Is it true that \m{13|52}?

		\item  Is it true that \m{27|9}?
		...
		\item  Let \m{a, b, c, d} be integers. Prove that if \m{a | b} and \m{c | d}, then \m{a c | b d}.

		\item  (transitive property of \m{|})  Let \m{a, b, c} be integers. Prove that if \m{a | b} and \m{b | c}, then \m{a | c}.
	\stopitemize
\stopslide

\startslide [title={Primes}]
	
	\startdefinition [title={prime, composite}]
		Let \m{p ∈ ℕ, p > 1}. Then \m{p} is called \emph{prime} if its only positive factors are \m{1} and \m{p}.

		A natural number \m{n > 1} is called composite if it is \emph{not} prime.
	\stopdefinition
	...
	\startitemize [m]
		...
		\item  (Linear combination lemma)  Let \m{d, m, n, x, y ∈ ℤ}. If \m{d | x} and \m{d | y}, then \m{d | (m x + n y)}.
	\stopitemize
