...
\startslide[title={Martingales and Markov processes}]
	...
	\startdefinition
		...
		Let \m{X_{\argdotsub}} is a \m{L^1}-bounded \m{ℱ_{\argdotsub}}-adapted stochastic process, and let \m{0 ≤ s ≤ t ≤ T}. Then, w.r.t. \m{ℱ_{\argdotsub}}, the process \m{X_{\argdotsub}} is called a
		\startitemize [5, joinedup]
			\item  \important{martingale} if \m{𝔼(X_t ∣ ℱ_s) = X_s},
			\item  \important{submartingale} if \m{𝔼(X_t ∣ ℱ_s) ≥ X_s}, and
	...
	\startremark
		...
		If \m{(X_n)} is a discrete-time martingale and \m{(H_n)} is an adapted process, then the process \m{Y_n = ∑_{j = 0}^{n - 1} H_n (X_{n + 1} - X_n) =: (H ∙ X)_n} is itself a martingale and called a \important{martingale transform} of \m{(X_n)}.
	\stopremark
...
\startslide [title={Itô integral for \m{X_{\argdotsub}} such that \m{∫_0^T X_t^2 \d t < ∞} a.s.}]
	...
	\startitemize [4]
		...
		% \item  TODO  Example: \m{∫_0^T e^{B_t^2} \d B_t = ∫_0^{B_1} e^{t^2} \d t - ∫_0^T B_t e^{B_t^2} \d t}.
...
\startslide [title={Exponential processes and the Girsanov theorem}]
	...
	\startitemize [4]
		...
		\item  Let \m{h_{\argdotsub}} be a stochastic process. The associated \important{exponential process} is defined as
			\startformula
		...
		\item  The \important{Girsanov theorem} \cite[short][Girsanov1960]:
			The translated stochastic process \m{W_{\argdotsub} = B_{\argdotsub} + ∫_0^{\argdotsup} h_t \d t} is a Brownian motion under the probability measure \m{\widetilde{ℙ}} defined by the Radon-Nikodym derivative
...
\startslide [title={Motivation}]
	...
	\startitemize [4]
		...
		\item  Iterated integrals: Consider the iterated integral \m{∫_0^T ∫_0^T \d B_s \d B_t \ugly{= ∫_0^T B_T \d B_t ≟ B_T^2}}.
		...
		\startitemize [5, joinedup]
			\item  Enlargement of filtration \m{𝓖_{\argdotsub} = 𝓕_{\argdotsub} ∨ B_1}, with Itô's decomposition of integrand

			\m{B_t = \brnd[B_t - ∫_0^t \frac{B_T - B_s}{T - s} \d s] + ∫_0^t \frac{B_T - B_s}{T - s} \d s}.
				% \startformula
...
\startslide [title={The near-martingale property}]
	...
	\startitemize [4]
		...
		\item  Answer for martingales: \quotation{near-martingales} \cite[short][KuoSaeTangSzozda2012].

		\item  Example: \m{𝔼(B_T B_t - t ∣ ℱ_s) = B_s^2 - s ≠ B_T B_s - s}, but \m{𝔼(B_T B_s - s ∣ ℱ_s) = B_s^2 - s}. \smile

		\item  Let \m{Z(t)} be a process such that \m{𝔼\abs[Z(t)] < ∞ \ ∀ t}, and \m{0 ≤ s ≤ t ≤ T}. Then \m{Z(t)} is called a
			\startitemize [5, joinedup]
	...
	\starttheorem [title={\cite[short][KuoSaeTangSzozda2012]}]
		Let \m{f, ϕ} be continuous function on \m{ℝ}. Under integrability conditions, the processes \m{X_{\argdotsub} = ∫_0^{\argdotsup} f(B_t) ϕ(B_T - B_t) \d B_t} and \m{Y^{\argdotsup} = ∫_{\argdotsub}^T f(B_t) ϕ(B_T - B_t) \d B_t} are near-martingales.
		% \startformula
...
\startslide [title={A generalized Itô formula \cite[short][HwangKuoSaitôZhai2016]}]
	...
	\starttheorem[title={\cite[short][HwangKuoSaitôZhai2016]}]
		...
		\okay{\m{\d Y^t = - η^t \d t - ς^t \d B_t}} be a \m{\tilde{d}}-dimensional \okay{instantly independent} process.
...
\startslide [title={Generalization of Itô isometry}]
	...
	\starttheorem [title={\cite[short][KuoSaeTangSzozda2013]}]
		Let \m{f, ϕ} be \m{C^1} function on \m{ℝ}. Then
		\startformula \startalign
...
\startslide [title={A generalization of Girsanov theorem}]
	...
	\starttheorem[title={\cite[short][KuoPengSzozda2013b]}]
		...
		Then translated stochastic process \m{W_{\argdotsub} = B_{\argdotsub} + ∫_0^{\argdotsup} (X_t + Y^t) \d t} is a near-martingale under the probability measure \m{\widetilde{ℙ}} defined by the Radon-Nikodym derivative \m{\frac{\d \widetilde{ℙ}}{\d ℙ} = 𝓔^{(X + Y)}_T}.
	\stoptheorem
\stopslide
...
\startslide [title={Applications of the Cramér theorem}]
	...
	\starttabulate [|M|M|M|]
		...
		\NC  \text{Exp}(λ)   \NC  \inv[{\brnd[1 - \frac{θ}{λ}]}]  \NC  \brnd[λx - 1 + x \log(λ x)] 𝟙_{[0, ∞)}(x) + ∞ 𝟙_{(-∞, 0)}(x)  \NR \NR
...
\startslide [title={The Schilder theorem}]
	...
	\startitemize [4]
		...
		\item  Let \m{\frac{1}{\sqrt{n}} B_t ∼ W^{(n)}}. Then \m{W^{(n)} = 𝓝\brnd[0, \frac{t}{n}] \xrightarrow{𝓓} δ_0} as \m{n → ∞}.
...
\startslide [title={The Freidlin–Wentzell theorem}]
	...
	\startitemize [4]
		...
		\item  As \m{n → ∞}, \m{W^{(n)}_x \xrightarrow{𝓓} δ_ξ}, where \m{ξ} solves the ODE \m{\dot{ξ}(t) = m(ξ(t)), \ ξ(0) = x}.
...
\startslide [title={Possible research directions}]
	...
	\startitemize [4]
		...
		\item  Prove Freidlin–Wentzell type results for stochastic differential equations with anticipating initial conditions.
