...
\startslide [title={Exponential processes and a generalized Girsanov theorem}]
	...
	\starttheorem[title={\cite[short][KuoPengSzozda2013b]}]
		Let \m{X_{\argdotsub}} and \m{Y^{\argdotsup}} be continuous square-integrable stochastic processes such that \m{X_{\argdotsub}} is adapted to \m{𝓕_{\argdotsub}}, and \m{Y^{\argdotsup}} is adapted to the backward Brownian filtration \m{𝓖^{\argdotsup}}.

		The translated stochastic process \m{W_{\argdotsub} = B_{\argdotsub} + ∫_0^{\argdotsup} (f_t + g^t) \d t} is a near-martingale under the probability measure \m{\widetilde{ℙ}} defined by the Radon-Nikodym derivative \m{\frac{\d \widetilde{ℙ}}{\d ℙ} = 𝓔^{(f + g)}_T}.
	\stoptheorem
\stopslide
