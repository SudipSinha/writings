...
\startchapter [title={Friedlin-Wentzell theorem for anticipating initial condition with extension of filtration}, reference=chp:FW-anticipating-IC-extension-filtration]
	...
	% 	\NC  \NC  =  \sqrt{ε} W_T - \sqrt{ε} ∫_0^t \d \tilde{W}_t - ∫_0^t \frac{Y_s^ε}{T - s} \d s  \NR
	% \stopalign  \stopformula
	...
	Note that we expect \m{Z_t^ε → 0} as \m{ε → 0}. In order to obtain a large deviation principle for \m{X_t^ε}, we first obtain the LDP for \m{Z_t^ε}.

	Firstly, we show that there is a unique solution of \m{Z_t^ε}. For convinience, let \m{\tilde{σ}_t (x, y) = (σ(x), -1)} and \m{\tilde{b}(x, y) = \frac{y}{T - s} (σ(x), -1)}.
	\startproposition
