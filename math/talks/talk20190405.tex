...
\startslide[title={What is it about?}]
	...
	\startitemize [4]
		...
		\item  A theory of characterization of rare events by providing exponential bounds on their probability.
		...
		\item  Example: A problem faced by the insurance industry.
			\startitemize [5, joinedup]
				...
				\item  \emph{Idea}: Choose \m{x} such that \m{ℙ\bcrl[\bar{X}_n > x]} is exponentially low.
			\stopitemize
	\stopitemize
\stopslide
...
\startslide [title={Insurance problem: large deviation bounds}]
	...
	\startitemize [n]
		...
		\item  We formally write \m{ℙ\bcrl[\bar{X}_n ∈ \d x] ≍ e^{-n I(x)} \d x} for \m{x ∈ ℝ}.
	\stopitemize
\stopslide
...
\startslide [title={Cramér theorem}]
	...
	\starttheorem [title={\cite[short][Cramér1938]}]
		Let \m{(X_n)} be a sequence of i.i.d. real random variables with finite moment generating function \m{M}. Then \m{(X_n)} follows LDP with rate function \m{I(x) = \sup_θ \brnd[θ x - \log M(θ)]}.
	\stoptheorem
...
\startslide [title={Freidlin–Wentzell theorem}]
	...
	\startitemize [4]
		...
		\item  Setup
			\startitemize [5, joinedup]
				\item  \m{∀ ε > 0}, let \m{X^{(ε)}_{\argdotsub}} be the solution of the SDE \m{\d X^{(ε)}_t = m(X^{(ε)}_t) \d t + σ(X^{(ε)}_t) \sqrt{ε} \d B_t , \ X^{(ε)}_0 = x}, where \m{m, σ} are sufficiently nice.
...
\startsection [title={Possible directions}, reference=sec:directions]
...
\startslide [title={Open problems}]
	...
	\startitemize [4]
		...
		\item  Freidlin–Wentzell type results for stochastic differential equations with anticipation.
		\item  Identification of the class of integrable processes under the new integral.
