...
\startslide [title={Wiener integral for \m{f ∈ L^2[0, T]}}]
	...
	\startitemize [4]
		...
		\item  The associated process \m{I_{\argdotsub} = ∫_0^∙ f(t) \d B_t} has the following properties.
			\startitemize [5, joinedup]
				\item  continuity
				\item  martingale
			\stopitemize
...
\startslide [title={Itô integral for \m{X_{\argdotsub} ∈ L^2_{\text{ad}} ([0, T] × Ω)}}]
	...
	\startitemize [4]
		...
		\item  The associated process \m{I_{\argdotsub} = ∫_0^∙ X_t \d B_t} has the following properties.
			\startitemize [5, joinedup]
				\item  continuity
				\item  martingale
			\stopitemize
...
\startslide [title={Multiple Wiener–Itô integrals}]
	...
	\startitemize [4]
		...
		\item  Naive idea: \m{∫_0^T ∫_0^T \d B_s \d B_t = ∫_0^T \d B_s ∫_0^T \d B_t = B_T^2}.

			But \m{𝔼 B_T^2 = \ugly{t ≠ 0}}, so \ugly{no martingale property}. \frown

		\item  Itô's idea: remove the diagonal to get \m{∫_0^T ∫_0^T \d B_s \d B_t  =  2 ∫_0^T ∫_0^t \d B_s \d B_t  =  B_T^2 - T}. \smile
...
\startslide[title={What is it about?}]
	...
	\startitemize [4]
		...
		\item  Informally a theory of characterization of rare events by providing exponential bounds on their probability.
		...
		\item  Example: A problem faced by the insurance industry.
			\startitemize [5, joinedup]
				\item  Value of claims received on day \m{n}: \m{X_n} \$.
				\item  Steady income from premium: \m{p} \$/day.
				\item  Planning period: \m{n} days.
				\item  Average expenditure: \m{\bar{X}_n = \frac1n ∑_{j = 1}^n X_j} \$/day.
				\item  \emph{Question}: How do we decide on the premium?
				\item  \emph{Idea}: Choose \m{p} such that the probability of the average expenditure to the company going above \m{p} is exponentially low.
			\stopitemize
	\stopitemize
\stopslide
...
\startslide [title={Insurance problem: setup}]
	...
	\startitemize [n]
		...
		\item  Asymptotic behavior of \m{\bar{X}_n} given by
			\startitemize [5, joinedup]
				\item  Weak law of large numbers: \m{\bar{X}_n \xrightarrow{ℙ} m}.
				\item  Central limit theorem: \m{\sqrt{n} (\bar{X}_n - m) \xrightarrow{𝓓} 𝓝(0, σ^2)}.
			\stopitemize

		\item  But at what rate?
...
\startslide [title={Insurance problem: large deviation bounds}]
	...
	\startitemize [n]
		...
		\item  We can also obtain a lower bound using an exponential change of measure
			\startformula
				\liminfm \frac1n \log ℙ\bcrl[\bar{X}_n ∈ E]  ≥  -\inf_{\intr[E]} I  \qquad  ∀ E ∈ 𝓑 .
			\stopformula

		\item  So we \quotation{formally} write \m{ℙ\bcrl[\bar{X}_n ∈ \d x] ≍ e^{-n I(x)} \d x} for \m{x ∈ ℝ}.
	\stopitemize
\stopslide
...
\startslide [title={Definition of large deviation principle}]
	...
	\startdefinition
		...
		\startformula \startalign[n=4]
			\NC  \text{(upper bound)}  \qquad  \NC  \limsupm \frac1n \log ℙ\bcrl[\bar{X}_n ∈ F]  \NC ≤  -\inf_{F} I  \NC  \qquad  ∀ F \text{ closed}  \NR
			\NC  \text{(lower bound)}  \qquad  \NC  \liminfm \frac1n \log ℙ\bcrl[\bar{X}_n ∈ E]  \NC ≥  -\inf_{G} I  \NC  \qquad  ∀ G \text{ open}
		\stopalign \stopformula
	\stopdefinition
\stopslide
...
\startslide [title={Laplace principle and equivalence to LDP}]
	...
	\startdefinition [title={Laplace principle}]
		...
		\startformula
			\lim  \frac1n \log 𝔼\brnd[\exp(-n h(X_n))]  =  \inf_𝓧 (h + I)
		\stopformula
	\stopdefinition

	\starttheorem
		\m{(X_n)} satisfies the Laplace principle on \m{𝓧} with rate function \m{I} if and only if \m{(X_n)} satisfies the large deviation principle on \m{𝓧} with rate function \m{I}.
	\stoptheorem

	\bold{Some other results}
...
\startslide [title={Cramér theorem}]
	...
	Rate function for some common distributions for \m{(X_n)}
	\placetable[force,none]{}{%    % This centers the table
	\starttabulate [|M|M|M|]
		\FL
		\NC  \text{Distribution}  \NC  M(θ)  \NC  I(x)  \NR
		\FL
		\NC  Bern(p)  \NC  1 - p + p e^θ  \NC  \brnd[x \log \frac{x}{1 - p} + (x - 1) \log \frac{p}{x - 1}] 𝟙_{[0, 1]}(x) + ∞ 𝟙_{[0, 1]^∁}(x)  \NR
		\NC  Pois(λ)  \NC  e^{λ(e^θ - 1)}  \NC  \brnd[λ - x + x \log \frac{x}{λ}] 𝟙_{[0, ∞)}(x) + ∞ 𝟙_{(-∞, 0)}(x)  \NR
		\NC  Exp(λ)   \NC  \inv[{\brnd[1 - \frac{θ}{λ}]}]  \NC  \brnd[λx - 1 + x \log(λ x)] 𝟙_{[0, ∞)}(x) + ∞ 𝟙_{(-∞, 0)}(x)  \NR
...
\startslide [title={Schilder theorem}]
	...
	\startitemize [4]
		...
		\startitemize [5, joinedup]
			...
			\item  \m{∀ ε > 0}, let \m{\sqrt{ε} B_t ∼ W_ε}. Then \m{W_ε = 𝓝(0, ε t) \xrightarrow{𝓓} δ_0} as \m{ε → 0}.
			% This scales down the variance of the Brownian motion to \m{ε t}.
			\item  Let \m{\text{CM} = \bcrl[ω ∈ C_0 : ω \text{ is absolutely continuous and } \dot{ω}_t ∈ L^2{[0, T]}]}.
		\stopitemize
	\stopitemize
	...
	\starttheorem
		...
		\startformula
			I(ω)  =  \brnd[\frac12 ∫_0^T {\abs[\dot{ω}(t)]}^2 \d t] 𝟙_{\text{CM}}(ω) + ∞ 𝟙_{\text{CM}^∁}(ω)
		\stopformula
	\stoptheorem
\stopslide
...
\startslide [title={Freidlin–Wentzell theorem}]
	...
	\startitemize [4]
		...
		\item  Setup
			\startitemize [5, joinedup]
				\item  Let \m{B_{\argdotsub}} be a \m{d}-dimensional Brownian motion.
				\item  \m{∀ ε > 0}, let \m{X^{(ε)}_{\argdotsub}} be the solution of the stochastic differential equation

					\m{\d X^{(ε)}_t = m(X^{(ε)}_t) \d t + σ(X^{(ε)}_t) \sqrt{ε} \d B_t , \ X^{(ε)}_0 = x}, where \m{m, σ} are sufficiently nice. Let \m{A = σ σ^*}.
				\item  Let \m{W^{(ε)}_x} denote the law of \m{X^{(ε)}_{\argdotsub}} starting at \m{x}.

				\item  As \m{ε → 0}, \m{W^{(ε)}_x \xrightarrow{𝓓} δ_ξ}, where \m{ξ} solves the ordinary differential equation

					\m{\dot{ξ}(t) = m(ξ(t)), \ ξ(0) = x}.
			\stopitemize
	\stopitemize
	...
	\starttheorem
		For \m{x} fixed, the family of probability measures \m{\bcrl[W^{(ε)}_x : ε > 0]} satisfy the large deviations principle with the rate function \m{I_x : C_0 → \clsr[ℝ]} given by
		\startformula
			I_x(ω)  =  \brnd[\frac12 ∫_0^T {\inn[\dot{ω}_t - b(ω_t), {\inv[A](ω_t)}{\brnd[\dot{ω}_t - b(ω_t)]}]} \d t] 𝟙_{\text{CM} ∩ \bcrl[ω_0 = x]}(ω) + ∞ 𝟙_{CM^∁ ∪ {\bcrl[ω_0 ≠ x]}}(ω) .
		\stopformula
	\stoptheorem
\stopslide
...
\startsection [title={Summary}, reference=sec:conclusion]
...
\startslide [title={Open problems}]
	...
	\startitemize [4]
		...
		\item  Extension to stochastic differential equations with anticipating coefficients
		\item  Freidlin-Wintzell type result for stochastic differential equations with anticipation
		\item  Identification of class of integrable processes under the generalized integral
