...
\startslide [title={Exercises}]
	...
	\startitemize [m]
		...
		\item  Let \m{n > 2}. Prove that there exists a prime p such that \m{n < p < n!}.
...
\startslide [title={GCD and LCM}]
	...
	\startdefinition
		Let \m{a, b ∈ ℤ, a ≠ 0 ≠ b}. The \important{greatest common divisor} (gcd) of \m{a} and \m{b} is the largest \m{d ∈ ℕ} for which \m{d ∣ a} and \m{d ∣ b}. We write \m{d = \gcd(a, b)}.

		Numbers whose gcd is 1 are called \important{coprime}.
	\stopdefinition
	
	Exercises	
	\startitemize [m]
		
		\item  Calculate the gcd and lcm of the following sets of numbers:
		...
		\item  Let n > 1 be an integer. Calculate the gcd and lcm of the following sets of numbers:
			\startitemize [i, columns, joinedup]
				\item  \m{\bcrl[n, 3 n]}.
				\item  \m{\bcrl[n, n + 1]}.
...
\startslide [title={Exercises}]
	...
	\startitemize [m]
		...
		\item  Consider the following numbers of the form \m{p^2 - 1}, where \m{p} is prime.
			...
			If \m{p > 3} is prime, then \m{12 ∣ (p^2 − 1)}.

		\item  You have to fill a room of size \m{175 × 65} with square tiles. What is the largest size of tiles you can use?
...
\startsection [title={Linear Diophantine equations}, reference=sec:linear-diophantine-eqns]
