...
\setvariables [document] [color-solution=ivory]    % Change to pink to see
...
\startslide [title={Some key points about the Euclidean algorithm}]
	...
	\startproof
		Follows from extended Euclidean algorithm.
	\stopproof
\stopslide
...
\startslide [title={Can the congruence relation be seen as equality?}]
	...
	\startitemize [m, joinedup]
		...
		\item  (addition)  If \m{a ≡_n b}, then \m{a + c ≡_n b + d}.
			\soln{T (\m{c ≡ c \ (\mod n ).})}

		\item  (multiplication)  If \m{a ≡_n b}, then \m{a c ≡_n b d}.
			\soln{T (\m{c ≡ c \ (\mod n ).})}
...
\startslide [title={Finding remainders}]
	...
	\startitemize [m]
		...
		\item  For \m{n ∈ ℕ}, use congruence theory to establish each of the following divisibility statements:
			\startitemize [i, columns, joinedup]
				\item  \m{13 ∣ \brnd[3^{n + 2} + 4^{2 n + 1}]}.
				\item  \m{7 ∣ \brnd[5^{2 n} + 3 ⋅ 2^{5 n - 2}]}.
			\stopitemize

		\item  Find all values of \m{x} (in the range \m{0, 1, ⋯ , 9}) that make the following congruences true.
			\startitemize [i, columns, joinedup]
				\item  \m{x + 8 ≡_{10} 0}.
				\item  \m{7 + 3 x ≡_{10} 0}.
				\item  \m{4 x + 6 ≡_{10} 0}.
...
\startslide [title={Equivalence relations}]
	...
	\startexample
		\startitemize[i, joinedup]
			...
			\item  The sets \m{\bcrl[3 k : k ∈ ℤ], \bcrl[3 k + 1: k ∈ ℤ], \bcrl[3 k + 2: k ∈ ℤ]} partition the set of integers. It corresponds to the relation \m{≡_3}.
		\stopitemize
	\stopexample
...
\startslide [title={Tests of divisibility: \m{9}}]

\stopslide

\startslide [title={Linear congruences}]

\stopslide

\startslide [title={The Chinese remainder theorem}]
